...
# import locale
# from odoo.addons import account_element as tools
...
class ReportAmountBalancesOptimized(models.Model):
	_name = 'report.amount.balances.optimized'
	...
	# date=fields.Date(string="Fecha de Cierre Contable")
	...
	report_amount_balances_optimized_line_ids = fields.One2many('report.amount.balances.optimized.line' , 'report_amount_balances_optimized_id' , string="Balance de Sumas y Saldos", readonly=True)

	partner_ids = fields.Many2many('res.partner','report_amount_balances_optimized_partner_rel','partner_id','report_amount_balances_optimized_id_1' ,string="Socios")
	partner_option=fields.Selection(selection=options , string="")#,default='in')

	account_ids = fields.Many2many('account.account','report_amount_balances_optimized_account_rel','account_id','report_amount_balances_optimized_id_2',string='Cuentas')
	account_option=fields.Selection(selection=options , string="")#,default='in')

	journal_ids = fields.Many2many('account.journal','report_amount_balances_optimized_journal_rel','journal_id','report_amount_balances_optimized_id_3',string="Diarios")
	journal_option=fields.Selection(selection=options , string="")#,default='in')

	move_ids = fields.Many2many('account.move','report_amount_balances_optimized_move_rel','move_id','report_amount_balances_optimized_id_4',string='Asientos Contables')
	move_option=fields.Selection(selection=options , string="")#,default='in')

	payment_ids = fields.Many2many('account.payment','report_amount_balances_optimized_payment_rel','payment_id','report_amount_balances_optimized_id_6',string="Pagos")
	payment_option=fields.Selection(selection=options , string="")#,default='in')

	####### FILTRO DE PERIODOS FISCALES !!!
	#period_fiscal_year_ids = fields.Many2many('account.period.fiscal.year','report_amount_balances_optimized_fiscalyear_rel','period_fiscal_year_id','report_amount_balances_optimized_id_7' ,string="Periodos Fiscales")
	#periods_option=fields.Selection(selection=options , string="")#,default='in')
	...
	column_balance_category=fields.Many2many('balance.category.column', 'report_amount_balances_optimized_column_rel','category_column_id','report_amount_balances_optimized_id_7',
		string="Elegir Columnas a Imprimir")

	###############################################
	existe_asiento_apertura=fields.Boolean(string="Existe Asiento de Apertura",default=True,compute="compute_campo_existe_asiento_apertura")
	##########################################
	...
	def file_name(self, file_format):
		...
		#file_name = "0710%s%s.%s" % (self.company_id.vat or '',self.fecha_inicio.split('-')[0] if self.fecha_inicio.split('-')[0] == self.fecha_final.split('-')[0] else (str(self.fecha_inicio.split('-')[0]) + "_" + str(self.fecha_final.split('-')[0])),file_format)
		return file_name
	...
	def action_print(self):
		if (self.print_format) :
			...
			raise UserError(_('NO SE PUEDE IMPRIMIR , El campo: Formato Impresión es obligatorio, llene ese campo !!'))
	...
	def query_balance_of_sums_and_balances(self,fecha_movimiento_debe,fecha_movimiento_haber):
		...
		if self.currency_id and self.currency_id != self.company_id.currency_id:
			query=unique_queries.query_account_amount_balances_with_period_currency(fecha_movimiento_debe,fecha_movimiento_haber,filter_clause)
		else:
			query=unique_queries.query_account_amount_balances_with_period(fecha_movimiento_debe,fecha_movimiento_haber,filter_clause)
	...
	def run_balance_of_sums_and_balances(self):
		...
		'''registro.append((0,0,{
				'name_account':'------------------------------',
				'saldos_iniciales_deudor':'----------------',
				'saldos_iniciales_acreedor':'----------------',
				'anio_fiscal_debe':'----------------',
				'anio_fiscal_haber':'----------------',
				'saldos_finales_deudor':'----------------',
				'saldos_finales_acreedor':'----------------',
				'balance_general_activo': '----------------',
				'balance_general_pas_y_patr':'----------------',
				'resultados_naturaleza_perdidas':'----------------',
				'resultados_naturaleza_ganancias':'----------------',
				'resultados_funcion_perdidas':'----------------',
				'resultados_funcion_ganancias':'----------------',
			}))'''
		...
		registro.append((0,0,{
				...
				# 'saldos_iniciales_deudor':'{:,.2f}'.format((abs(resultado[0]) if resultado[0]>=0 else 0.00) + saldo[0]),
				# 'saldos_iniciales_acreedor':'{:,.2f}'.format((abs(resultado[0]) if resultado[0]<0 else 0.00) + saldo[1]),
				# 'anio_fiscal_debe':'{:,.2f}'.format((abs(resultado[1]) if resultado[1]>=0 else 0.00) + saldo[2]),
				# 'anio_fiscal_haber':'{:,.2f}'.format((abs(resultado[1]) if resultado[1]<0 else 0.00) + saldo[3]),
				# 'saldos_finales_deudor':'{:,.2f}'.format((abs(resultado[2]) if resultado[2]>=0 else 0.00) + saldo[4]),
				# 'saldos_finales_acreedor':'{:,.2f}'.format((abs(resultado[2]) if resultado[2]<0 else 0.00) + saldo[5]),
