...
class HrLoan(models.Model):
    ...
    @api.depends('loan_amount', 'loan_lines')
    ...
    contract_id = fields.Many2one('hr.contract', related="employee_id.contract_id", readonly=True,
                                    string="Contrato", help="Employee")
    ...
    def action_draft(self):
        if self.total_paid_amount == 0:
            ...
            raise ValidationError("El préstamo no se puede establacer al estado borrador porque ya existen nóminas asocias")
    ...
    # @api.constrains('total_amount', 'loan_lines')
    def _check_loan_amount(self):
        for loan in self:
            if loan.loan_lines:
                total_amount = sum(loan.loan_lines.mapped('amount'))
                if loan.loan_amount != total_amount:
                    raise ValidationError('El importe del préstamo no coincide con la suma de los importes de las cuotas.')
